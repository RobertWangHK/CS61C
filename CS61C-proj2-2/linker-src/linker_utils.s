...
relocate_inst:
	...
	move $s3, $a3 #relocate_table
	
	move $a0, $s3 
	move $a1, $s1 #addr to look for
	jal symbol_for_addr
	beq $v0, $0, error
	move $a0, $s2 #pointer to symbol table
	move $a1, $v0 #the symbol name found
	jal addr_for_symbol
	beq $v0, -1, error
	move $t0, $v0 # put the address found in relocation table into $t0
	
	#andi $t0, $t0, 0x0FFFFFFC
	move $t1, $s0 #t1 is the instruction
	srl $t2, $t1, 26
	sll $t2, $t2, 26
	srl $t0, $t0, 2
	or $s0, $t2, $t0
