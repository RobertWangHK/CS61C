...
write_machine_code:
	...
	sw $s4, 4($sp)
	sw $s5, 24($sp)
	sw $s6, 28($sp) #instruction in number
	sw $s7, 32($sp)
	sw $ra, 0($sp)
...
write_machine_code_find_text:	
	...
	li $s5, 0		#initialize byte offset
write_machine_code_next_inst:
	# 2. Call readline() while passing in the correct arguments:
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s1		#a0 is file handle so it should be the input file
	jal readline
...
write_machine_code_end:
	...
	lw $ra, 0($sp)
	lw $s5, 24($sp)
	lw $s6, 28($sp) #instruction in number
	lw $s7, 32($sp)
	addiu $sp, $sp, 36
	jr $ra
