...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        char name[10] = "";
        int result = add_if_label(input_line, token, byte_offset, symtbl);
        if(result == 0){
            //the first one is not lable, treat it as instruction name
            strcpy(name, token);
        }
        else{
            if(result==-1){
            ...
            //the first is a valid token, treat next as instruction name
            token = strtok(NULL, IGNORE_CHARS); //move to the next one
        ...
        char* temp = strtok(NULL, IGNORE_CHARS);

        while (temp!=NULL){
            args[num_args]=temp;
            num_args++;   
            temp = strtok(NULL, IGNORE_CHARS);
            if(add_if_label(input_line, token, byte_offset, symtbl)!=0){
                ret_code=-1;
            }
        }
        if(num_args>3){
